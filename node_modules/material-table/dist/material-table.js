...
import { Paper, Table, TableRow, TableFooter, TablePagination, withStyles } from '@material-ui/core';
...
class MaterialTable extends React.Component {
  ...
  getProps(props) {
    const calculatedProps = _objectSpread({}, props || this.props);
  ...
  getRenderData(data, props) {
    data = data || this.state.data;
    props = this.getProps();
    let renderData = [...data]; // App filter
    ...
    if (this.state && this.state.searchText) {
      renderData = renderData.filter(row => {
        let result = false;
        this.state.columns.filter(columnDef => {
  ...
  renderFooter() {
    ...
    if (props.options.paging) {
      ...
      }, React.createElement(TableRow, null, React.createElement(TablePagination, {
        style: {
          float: 'right'
        },
        colSpan: 3,
  ...
  render() {
    const props = this.getProps();
    return React.createElement(Paper, null, props.options.toolbar && React.createElement(MTableToolbar, {
      actions: props.actions,
      selectedRows: this.state.selectedCount > 0 ? this.state.data.filter(a => {
      ...
      onColumnsChanged: columns => this.setState({
        columns
      }),
